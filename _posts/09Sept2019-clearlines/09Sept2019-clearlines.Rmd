...
First of all lets generate data to provide a concrete common example.

```{r}
df <- data.frame(
  Treatment = rep(c("A", "B", "C", "D", "E", "F"),
                  each = 4),
  trt2 = rep(c("A", "B", "C", "D", "E", "F"),
             each = 4),
...
Plot all treatments with differing lines and symbols to ensure differentiation. 
...
```{r}
ggplot(df, aes(x = Visit, y = Response, group = Treatment)) +
  geom_line (
    data = transform(df, Treatment = NULL),
    aes(x = Visit, y = Response, group = trt2),
    size = 0.65,
    color = "darkgray"
  ) +
